...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your myrestfulapi account.<br>
        ...
        &mdash; myrestfulapi Team
      `
      return sendMail({ toEmail: email, subject: 'myrestfulapi - Password Reset', content })
    })
